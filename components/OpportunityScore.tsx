import { OpportunityBreakdown } from '@/types';

const LABELS: Record<keyof OpportunityBreakdown, string> = {
  audienceSize: 'Audience size',
  audienceFit: 'Audience fit',
  contentGap: 'Content gap',
  postingSafety: 'Posting safety',
  growthTrend: 'Growth trend',
};

export default function OpportunityScore({
  breakdown,
  total,
}: {
  breakdown: OpportunityBreakdown;
  total: number;
}) {
  return (
    <div className="bg-surface border border-cyan-border rounded-none p-5">
      <div className="text-t2 text-xs font-semibold uppercase tracking-widest mb-4">Opportunity Score</div>
      <div className="text-center mb-5">
        <div className="text-5xl font-extrabold text-hot">{total.toFixed(1)}</div>
        <div className="text-t2 text-xs mt-1">
          {total >= 8 ? 'High opportunity' : total >= 6 ? 'Moderate opportunity' : 'Low opportunity'}
        </div>
      </div>
      <div className="space-y-2.5">
        {(Object.entries(breakdown) as [keyof OpportunityBreakdown, number][]).map(([key, val]) => (
          <div key={key} className="flex items-center gap-3">
            <span className="text-t2 text-xs w-28 flex-shrink-0">{LABELS[key]}</span>
            <div className="flex-1 h-1.5 bg-overlay rounded overflow-hidden">
              <div
                className="h-full rounded bg-hot opacity-60"
                style={{ width: `${(val / 10) * 100}%` }}
              />
            </div>
            <span className="text-t1 text-xs font-semibold w-6 text-right">{val.toFixed(1)}</span>
          </div>
        ))}
      </div>
    </div>
  );
}
