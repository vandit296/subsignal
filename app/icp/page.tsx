...
export default function IcpRadarPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch('/api/icp-radar');
  ...
  return (
    <div style={{ maxWidth: 860, margin: '0 auto', padding: '26px 0 80px', fontFamily: UI }}>
      ...
      <div style={{ fontSize: 12, color: 'var(--t3)', margin: '6px 0 20px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12, marginBottom: 22 }}>
        {[['DELIVERED TODAY', batch?.stats.delivered ?? 0], ['DM TODAY (≥85)', dmToday], ['AVG FIT', avg]].map(([k, v]) => (
